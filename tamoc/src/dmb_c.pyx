...
# cdef int DP = 8
FLOAT_TYPE = cnp.float64
COMPLEX_TYPE = cnp.complex128
...
cpdef cubic_roots(cnp.ndarray[FLOAT_TYPE, ndim=1] p,
                  # x0 is output
                  # cnp.ndarray[FLOAT_TYPE, ndim=1] x0
                  ):
...
"""
    ...
    !
"""
    # maybe use this to fix the length of the arrays?
    cdef int N = 3
...
#     integer :: k
    cdef int k
#     real(kind = DP) :: Delta, a, b, c, d, eps
    cdef FLOAT_TYPE Delta, a, b, c, d, eps
#     real(kind = DP), dimension(N) :: Dk
    FLOAT_TYPE[N]
#     complex(kind = DP), parameter :: I = cmplx(0.0, 1.0)
    COMPLEX_TYPE I = 0.0+1.0j
#     complex(kind = DP) :: C0
    COMPLEX_TYPE C0 = 0.0+1.0j
#     complex(kind = DP), dimension(N) :: u
    COMPLEX_TYPE u = 0.0+1.0j

    cdef cnp.ndarray[FLOAT_TYPE, ndim=1] x0 = np.zeros((N,), dtype=np.complex128) # used for output
#     ! Extract the coefficient values for easier reference and understanding
#     ! in the code
    a = p[1]
    b = p[2]
